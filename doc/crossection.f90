...
!---------------------------------------------------------------
  ...
  read (*,'(a)') fileout
  !
  open (unit=7,file=fileout,status='unknown',form='formatted')
  write (7,"('# mesh=',i4,', xmin=',f7.2,', lmax=',i1)") mesh, xmin, lmax
  write (7,"('#     E          sigma(E)     sigma_l(E), l=0, lmax')") 
  !
  ! allocate arrays
  !
  ...
  do n=1,ne
     ...
     e = elw + (n-1)*de
     q = sqrt(e/hbar2m)
     ! lambda2 = 1/4 of the wavelength
     ! seems to prevent strange numerical instabilities when finding delta
     ! that was present with the previous choice (half the wavelength)
     lambda2 = pi/q/2.0_dp
     !
     rmin  = xmin
     ...
     do l=0,lmax
        ...
        !
        !do i=0,mesh
        !   write (10+l,'(f10.4,3e16.8)') &
        !       r(i), u(i), p(i), vpot(i)+hbar2m*l*(l+1)/r(i)**2
        !enddo
        !write (10+l,'(/)')
        !
     enddo
     ! max l: 13, or else the lines will be wrapped
     write (7,'(2f12.6,13e11.3)') e, SUM(cross), cross(:)
     !
   end do
   close(unit=7)
   !
